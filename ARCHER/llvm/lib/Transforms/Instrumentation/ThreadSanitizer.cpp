...
bool ThreadSanitizer::runOnFunction(Function &F) {
  ...
  if (ClInstrumentMemoryAccesses && SanitizeFunction)
...
#ifdef ARCHER
      if (!Inst->getMetadata(LLVMContext::MD_archer))
#endif // ARCHER
	Res |= instrumentLoadOrStore(Inst, DL);
        ...
        Inst->setMetadata(LLVMContext::MD_archer, N);
      }
#endif // ARCHER
    }
...
void ThreadSanitizer::makeInstructionsList(SmallVectorImpl<Instruction*> &All)
{
  ...
  if(!File.empty()) {
    StringRef FileName = "/tmp/blacklist.bl";

    std::error_code EC;
    tool_output_file F(FileName, EC, sys::fs::F_Append);
...
void ThreadSanitizer::makeFunctionsList(Function &F, Instruction &I)
{
  ...
  if (DILocation *Loc = I.getDebugLoc()) {
    ...
    std::error_code EC;
    tool_output_file File(FileName, EC, llvm::sys::fs::F_Append);
