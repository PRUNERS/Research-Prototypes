...
static bool isAtomic(llvm::Instruction *I) {
  ...
  if (isa<llvm::AtomicRMWInst>(I))
    return true;
  if (isa<llvm::AtomicCmpXchgInst>(I))
    return true;
  if (isa<llvm::FenceInst>(I))
    return true;
  return false;
}
#endif // ARCHER
...
void CodeGenModule::Release() {
  ...
  for (llvm::Function &F : getModule()) {
    for (auto &BB : F) {
      for (auto &Inst : BB) {
        if((isAtomic(&Inst)) || (isa<llvm::LoadInst>(Inst) || isa<llvm::StoreInst>(Inst)) \
           || ((isa<llvm::CallInst>(Inst) || isa<llvm::InvokeInst>(Inst)) && (isa<llvm::MemIntrinsic>(Inst)))) {
          if(SanitizerBL.isBlacklistedLine(Inst)) {
            llvm::LLVMContext& C = Inst.getContext();
            llvm::MDNode *N = llvm::MDNode::get(C, llvm::MDString::get(C, "not instrument"));
            Inst.setMetadata(llvm::LLVMContext::MD_archer, N);
          }
        }
      }
    }
