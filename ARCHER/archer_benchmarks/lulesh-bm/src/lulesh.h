...
#define SIZE_ELEM SIZE * SIZE * SIZE
#define SIZE_NODE (SIZE + 1) * (SIZE + 1) * (SIZE + 1)
#define SIZE_NODEBC (SIZE + 1) * (SIZE + 1)
#define ALL_ELEM SIZE_ELEM + (SIZE * SIZE * 2) + (SIZE * SIZE * 2) + (SIZE * SIZE * 2)
...
class Domain {
   ...
   bool symmXempty()          { if(sizeof(m_symmX) == 0) return true; return false; }
   bool symmYempty()          { if(sizeof(m_symmY) == 0) return true; return false; }
   bool symmZempty()          { if(sizeof(m_symmZ) == 0) return true; return false; }
 ...
 public:
   /* Node-centered */
   Real_t m_x[SIZE_NODE] ;  /* coordinates */
   Real_t m_y[SIZE_NODE] ;
   Real_t m_z[SIZE_NODE] ;

   Real_t m_xd[SIZE_NODE] ; /* velocities */
   Real_t m_yd[SIZE_NODE] ;
   Real_t m_zd[SIZE_NODE] ;

   Real_t m_xdd[SIZE_NODE] ; /* accelerations */
   Real_t m_ydd[SIZE_NODE] ;
   Real_t m_zdd[SIZE_NODE] ;

   Real_t m_fx[SIZE_NODE] ;  /* forces */
   Real_t m_fy[SIZE_NODE] ;
   Real_t m_fz[SIZE_NODE] ;

   Real_t m_nodalMass[SIZE_NODE] ;  /* mass */

   Index_t m_symmX[SIZE_NODE] ;  /* symmetry plane nodesets */
   Index_t m_symmY[SIZE_NODE] ;
   Index_t m_symmZ[SIZE_NODE] ;
   ...
   Index_t  m_nodelist[8*SIZE_ELEM] ;     /* elemToNode connectivity */

   Index_t  m_lxim[SIZE_ELEM] ;  /* element connectivity across each face */
   Index_t  m_lxip[SIZE_ELEM] ;
   Index_t  m_letam[SIZE_ELEM] ;
   Index_t  m_letap[SIZE_ELEM] ;
   Index_t  m_lzetam[SIZE_ELEM] ;
   Index_t  m_lzetap[SIZE_ELEM] ;

   Int_t    m_elemBC[SIZE_ELEM] ;  /* symmetry/free-surface flags for each elem face */

   Real_t m_dxx[SIZE_ELEM] ;  /* principal strains -- temporary */
   Real_t m_dyy[SIZE_ELEM] ;
   Real_t m_dzz[SIZE_ELEM] ;

   Real_t m_delv_xi[ALL_ELEM] ;    /* velocity gradient -- temporary */
   Real_t m_delv_eta[ALL_ELEM] ;
   Real_t m_delv_zeta[ALL_ELEM] ;

   Real_t m_delx_xi[SIZE_ELEM] ;    /* coordinate gradient -- temporary */
   Real_t m_delx_eta[SIZE_ELEM] ;
   Real_t m_delx_zeta[SIZE_ELEM] ;
   
   Real_t m_e[SIZE_ELEM] ;   /* energy */

   Real_t m_p[SIZE_ELEM] ;   /* pressure */
   Real_t m_q[SIZE_ELEM] ;   /* q */
   Real_t m_ql[SIZE_ELEM] ;  /* linear term for q */
   Real_t m_qq[SIZE_ELEM] ;  /* quadratic term for q */

   Real_t m_v[SIZE_ELEM] ;     /* relative volume */
   Real_t m_volo[SIZE_ELEM] ;  /* reference volume */
   Real_t m_vnew[SIZE_ELEM] ;  /* new relative volume -- temporary */
   Real_t m_delv[SIZE_ELEM] ;  /* m_vnew - m_v */
   Real_t m_vdov[SIZE_ELEM] ;  /* volume derivative over volume */

   Real_t m_arealg[SIZE_ELEM] ;  /* characteristic length of an element */
   
   Real_t m_ss[SIZE_ELEM] ;      /* "sound speed" */

   Real_t m_elemMass[SIZE_ELEM] ;  /* mass */
