...
int main (int argc, char **argv) {
  ...
  /* Default: DEFAULT_NPARTS, DEFAULT_NSTEPS */
	...
	printf("p=%p, v=%p, f=%p, a=%p, p-v=0x%llx, f-v=0x%llx, a-v=0x%llx\n",position, velocity, force, accel, (long long)position-(long long)velocity, (long long)force-(long long)velocity, (long long)accel-(long long)velocity);
  ...
  for (i = 0; i < NSTEPS; i++) {
    compute(NPARTS, NDIM, position, velocity, mass, force, &potential, &kinetic);
//#if 0 
    printf("%17.9e %17.9e %17.9e\n", potential, kinetic, (potential - E0 + kinetic) / E0);
//#endif
