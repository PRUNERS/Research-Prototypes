					 ...
					 const int switchThresh)
{
	...
	while (true) {
		while ((*myVec)[++i] < pivot && i<r);
		while ((*myVec)[--j] > pivot && j>q);
		if (i >= j)
			break;
	 ...
	 */
#		pragma omp task firstprivate(q, r, i, myVec, switchThresh)
		{
...
//	omp_set_num_threads(numThreads);
	...
	myVecP = &myVec;
#	pragma omp parallel sections firstprivate (myVecP, size, switchThresh)
	{
//#			pragma omp single
			{
				myQuickSort(myVecP, 0, size, switchThresh);
			}
	}
