...
void nd_exchange_ghost_cells() {
    ...
    MPI_Isend(sbufdown, nxl, MPI_DOUBLE, down, 0, MPI_COMM_WORLD, &req[0]);

    for(i = 1; i <=nxl; ++i) sbufup[i-1] = u[i][nyl];
    MPI_Isend(sbufup, nxl, MPI_DOUBLE, up, 0, MPI_COMM_WORLD, &req[1]);

    for(j = 1; j <= nyl; ++j) sbufleft[j-1] = u[1][j];
    MPI_Isend(sbufleft, nyl, MPI_DOUBLE, left, 0, MPI_COMM_WORLD, &req[2]);

    for(j = 1; j <= nyl; ++j) sbufright[j-1] = u[nxl][j];
    MPI_Isend(sbufright, nyl, MPI_DOUBLE, right, 0, MPI_COMM_WORLD, &req[3]);

    MPI_Barrier(MPI_COMM_WORLD);


    while(iter < 4) {
        MPI_Probe(MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &recvstat);
        src = recvstat.MPI_SOURCE;
        if(src == up && !isUp) {
            MPI_Recv(rbuff, nxl, MPI_DOUBLE, up, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            for(i = 1; i <= nxl; ++i) u[i][nyl+1] = rbuff[i-1];
	    isUp = 1;
        }
        else if(src == down && !isDown) {
            MPI_Recv(rbuff, nxl, MPI_DOUBLE, down, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            for(i = 1; i <= nxl; ++i) u[i][0] = rbuff[i-1];
	    isDown = 1;
        }
        else if(src == right && !isRight) {
            MPI_Recv(rbuff, nyl, MPI_DOUBLE, right, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            for(j = 1; j <= nyl; ++j) u[nxl+1][j] = rbuff[j-1];
            isRight = 1;
        }
        else if(src == left && !isLeft) {
            MPI_Recv(rbuff, nyl, MPI_DOUBLE, left, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
