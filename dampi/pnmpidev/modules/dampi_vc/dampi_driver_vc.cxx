...
#define PNMPI_MODULE_DAMPI_DRIVER "dampi_driver_vc"
...
#define PBGET_AND_SET(res)                                        \
  if (res==MPI_SUCCESS) {                                         \
    pb_get(pb_size,pbdata);                                       \
    int* incoming_epoch=(int*)malloc(pb_size);                    \
    memcpy(incoming_epoch,pbdata,pb_size);                        \
    for(int __i=0;__i<num_procs;__i++){                             \
        COMPARE_AND_SET(curr_epoch[__i],incoming_epoch[__i]);}    \
    free(incoming_epoch);}
#endif

#define COPY_CLOCK(to,from) memcpy((to),(from),pb_size)
#define UPDATE_CLOCK(incoming)                        \
  for (int __i=0;__i<num_procs;__i++){                        \
    COMPARE_AND_SET(curr_epoch[__i],incoming[__i]);}
#define ADVANCE_MYCLOCK curr_epoch[myrank]++
                     ...
                     MPI_Comm comm, MPI_Request request)
{
  ...
  eventmap[eventNo]->start_epoch = (int*) malloc(num_procs * sizeof(int));
  COPY_CLOCK(eventmap[eventNo]->start_epoch,start_epoch);

  eventmap[eventNo]->end_epoch = (int*) malloc(num_procs * sizeof(int));
#ifndef EARLY_IRECV_INCREMENT
  COPY_CLOCK(eventmap[eventNo]->end_epoch,end_epoch);
#else
  COPY_CLOCK(eventmap[eventNo]->end_epoch,start_epoch);
#endif

  if (eventType == IRECV_EVENT) eventmap[eventNo]->end_epoch[myrank] = MAX_EPOCH;
...
void DumpEpochDataMaster() {
  ...
  if (event_list->size() == 0)
    ...
    for (; it != it_end; it++) {
      ...
      mycount += num_procs; /* size of vector clock */
      mycount += 1; /* space for event number */
  ...
  if (event_list->size() == 0) {
    ...
    for (; it != it_end; it++) {
      ...
      /* first the epoch */
      for (int i = 0; i < num_procs; i++)
        mydata[mydata_index++] = event->end_epoch[i];
  ...
  if (myrank == MASTER_NODE) {
    ...
    /* Print the final vector clock */
    for (int i = 0; i < num_procs; i ++)
      fprintf(outputFile, "%d ", curr_epoch[i]);
    fprintf(outputFile, "\n");
...
/* Pre: vclk is a valid vector clock
 * Post: return whether this vector clock is past the guided 
 * point provided by the scheduler 
 */
bool isPastGuidedPoint(int* vclk) {
  assert (vclk != NULL);
  return vclk[myrank] > last_guided_epoch;
}
...
bool FindPotentialMatches(int source, int tag, MPI_Comm comm, 
  ...
  for (; it != it_rend; it++) {
    EventData_t* event = eventmap[*it];
    if (event->end_epoch[myrank] >= incoming_epoch &&  /* don't bother if the message is not late wrt to this event */
        event->end_epoch[myrank] <= matching_epoch &&
        *it <= my_eventNo && /* only checking for Recvs issued before the one matched with this msg */
        isPastGuidedPoint(event->end_epoch) && /* don't worry about the ones before the guided epoch */
        (event->tag == tag || event->tag == MPI_ANY_TAG) &&
        event->comm == comm &&
        event->end_epoch[myrank] != MAX_EPOCH) /* the ones with MAX_EPOCH were taken care of earlier */
      {      /* found late message */
    ...
    else if (event->end_epoch[myrank] < incoming_epoch 
             || !isPastGuidedPoint(event->end_epoch)) 
      break;
  }
...
void CompletePrecedingIrecv(MPI_Status* status, MPI_Comm comm,
  ...
  while (it != pending_irecv_list.end()) {
    ...
    if ((event->tag == (*status).MPI_TAG ||
      ...
      COPY_CLOCK(event->end_epoch,curr_epoch);
      ADVANCE_MYCLOCK;
...
void UpdateIrecvClock(PNMPIMOD_Requests_Parameters_t req_param) {
  ...
  if (req_param.node == MPI_ANY_SOURCE) {
    map<MPI_Request,int>::iterator it = req_to_event_map.find(req_param.inreq);
    if (eventmap[(int) it->second]->end_epoch[myrank] == MAX_EPOCH) {
      COPY_CLOCK(eventmap[(int) it->second]->end_epoch,curr_epoch);
      ADVANCE_MYCLOCK;
    }
  }
}
...
bool CheckPendingReceiveForMatches(MPI_Status* status, MPI_Comm comm, 
  ...
  for (;it != it_end; it++) {
    ...
    if (*it <= my_eventNo &&
      ...
      /* sanity check */
      assert (event->end_epoch[myrank] == MAX_EPOCH);
      list<int>::iterator mp_it = find(event->matching_procs->begin(),
                                      event->matching_procs->end(),
                                       (*status).MPI_SOURCE);
      if (mp_it == event->matching_procs->end()) {
...
void ProcessIncomingMessage(int err, int* incoming_epoch,
  ...
  if (pending_irecv_list.size() >0) {
    if (req_param == NULL)
      CheckPendingReceiveForMatches(status,comm, eventNo);
    else 
      CheckPendingReceiveForMatches(status,comm,
                                    req_to_event_map.find(req_param->inreq)->second);
  }

#endif
  UPDATE_CLOCK(incoming_epoch);
  if (curr_epoch[myrank] > incoming_epoch[myrank]) {
    /* blocking Recv, use current eventNo*/
    if (req_param == NULL) 
      FindPotentialMatches((*status).MPI_SOURCE, (*status).MPI_TAG, 
                           comm, curr_epoch[myrank]-1, incoming_epoch[myrank], eventNo);
    /* Async. Irecv */
    else {
      ...
      if (find(event_list->begin(), event_list->end(), it->second) != event_list->end())
        matching_epoch = eventmap[it->second]->end_epoch[myrank];
      else {/* this request was for a deterministic receive */
        matching_epoch = req_to_epoch_map.find(req_param->inreq)->second;
      }
      if (matching_epoch > incoming_epoch[myrank])
        FindPotentialMatches((*status).MPI_SOURCE, (*status).MPI_TAG,
                             req_param->comm, matching_epoch, incoming_epoch[myrank], it->second);
    }
  }
}
...
int MPI_Init(int *argc, char ***argv)
{
  ...
  pb_size = (size_t) num_procs * sizeof(int);
  pbdata=(char*) malloc((int)pb_size);
  if (pbdata==NULL) return MPI_ERROR_MEM;
  curr_epoch = (int*) calloc (num_procs,sizeof(int));
  initializeFileIO();
  return err;
}
...
int MPI_Init_thread(int *argc, char ***argv, int required, int *provided)
{
  ...
  */
  ...
  pbdata=(char*) malloc((int)pb_size);
  if (pbdata==NULL) return MPI_ERROR_MEM;
  curr_epoch = (int*) malloc (num_procs * sizeof(int));
...
{
  int err;
  if (isPastGuidedPoint(curr_epoch)) {
  ...
  if (source == MPI_ANY_SOURCE) {
...
#ifndef EARLY_IRECV_INCREMENT
      RecordEventData(eventNo,IRECV_EVENT,curr_epoch,curr_epoch,count,
    ...
    req_to_epoch_map[*request]=curr_epoch[myrank];
